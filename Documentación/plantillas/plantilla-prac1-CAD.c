...
void init_ADC(void)
{
    AD1CON1bits.AD12B = ; /* 10 bit conversión */
    AD1CON2bits.VCFG = ; /* Vdd and Vss as voltage references */
    AD1CON3bits.ADRC = ; /* clock derived from system clock */
    AD1CON3bits.ADCS = ; /* Clock for a TAD of 2 us */
    AD1CHS0bits.CH0SA = ; /* Channel 0 positive input as AN2 */
    AD1CHS0bits.CH0NA = ; /* Channel 0 negative input as -Vref */
    AD1CON2bits.CHPS = ; /* Channel 0 selected for sample and conversion */
    AD1CON2bits.CSCNA = ; /* Do not scan inputs */
    AD1CON1bits.ASAM = ; /* Sample begins when SAMP bit is set */
    AD1CON1bits.SAMP = ;
    AD1CON1bits.SSRC = ; /* Clearing sample bit ends sampling and start conversion */
    AD1CON1bits.FORM = ; /* Output format = integer */
    AD1CON2bits.BUFM = ; /* Always starts filling buffer at address 0 */
    AD1CON2bits.ALTS = ; /* Always uses channel input selects for Sample A */
    AD1CON2bits.SMPI = ; /* One conversion per interrupt */
...
int main(void) 
{
    ...
    AD1CHS0bits.CH0SA = 2; // Potentiometer
    while(1)
    {
